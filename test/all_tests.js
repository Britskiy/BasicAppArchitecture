...
const user_1 = require("../dist/models/user");
const user_2 = require("./data/user");
let expect = chai.expect;
let should = chai.should;
chai.use(chaiHttp);
//Clean
user_1.userSchemaModel.remove({}).exec((data) => { });
...
describe('User', () => {
    ...
    it('get user public info', (done) => {
        chai.request(server)
            .get('/user/' + user_id)
            .end(function (err, res) {
            expect(res.body.result.firstName).to.be.equal(user_2.userData.firstName);
            expect(res.body.isError).to.be.false;
            expect(res).to.have.status(200);
            done();
        });
    ...
    it('auth', (done) => {
        chai.request(server)
            .post('/user/auth')
            .send({ login: user_2.userData.login, password: user_2.userData.password })
            .end(function (err, res) {
    ...
    it('update with auth token', (done) => {
        let test_user = JSON.parse(JSON.stringify(user_2.userData));
    ...
    it('delete with auth token and password', (done) => {
        chai.request(server)
            .delete('/user')
            .send({ password: user_2.userData.password })
            .set('Authorization', 'Bearer ' + token)
            .end(function (err, res) {
